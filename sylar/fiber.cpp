...
void Fiber::call() {
    // SYLAR_LOG_DEBUG(g_logger) << " call  id=" << m_id;
    SetThis(this);
    m_state = EXEC;
...
void Fiber::back() {
    ...
    // SYLAR_LOG_DEBUG(g_logger) << " back  id=" << m_id;
}
...
void Fiber::CallerMainFunc(){
    ...
    cur.reset();
    a->back();
    SYLAR_ASSERT2(false, "never reach  " + std::to_string(a->m_id));
}
