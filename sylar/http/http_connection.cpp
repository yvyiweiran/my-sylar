...
HttpResult::ptr HttpConnection::DoGet(const std::string& uri
    ...
    Uri::ptr u = sylar::Uri::Create(uri);
    if(!u) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_URL
                , nullptr, "invalid uri");
    }
    return DoGet(u, timeout_ms, headers, body);
}
...
HttpResult::ptr HttpConnection::DoPost(const std::string& uri
    ...
    return DoPost(u, timeout_ms, headers, body);
}
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method
    ...
    return DoRequest(method, u, timeout_ms, headers, body);
}
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(!sock) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SOCKTE_CREATE_ERROR
    ...
    if(rt == 0) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_PEER
                , nullptr, "send request closed by peer: " + addr->toString());
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection(){
    ...
    while(!m_conns.empty()) {
        ...
        if((conn->m_create_time + m_maxAliveTime) > now_ms) {
            invalid_conn.push_back(conn);
            continue;
        }
        ptr = conn;
    ...
    if(!ptr) {
        ...
        if(!sock->connect(addr)) {
            SYLAR_LOG_ERROR(g_logger) << "connect sock fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    if(!ptr->isConnected() || 
    ptr->m_create_time + pool->m_maxAliveTime >= sylar::GetCurrentMS()) {
        delete ptr;
        --pool->m_total;
        return;
    }
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
    ...
    if(rt == 0) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_PEER
                , nullptr, "send request closed by peer:" + m_host + " port:" + std::to_string(m_port));
    }
