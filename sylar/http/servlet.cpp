#include "servlet.h"
#include <fnmatch.h> // fnmatch() 模糊匹配
...
int32_t ServletDispatch::handle(sylar::http::HttpRequest::ptr request
    , sylar::http::HttpResponse::ptr response
    , sylar::http::HttpSession::ptr session) {
    auto slt = getMathchedServlet(request->getPath());
    if(slt) {
...
Servlet::ptr ServletDispatch::getMathchedServlet(const std::string& uri) {
    RWMutexType::ReadLock lock(m_mutex);
    auto it = m_datas.find(uri);
    if(it != m_datas.end()) {
        return it->second;
    }
    for(auto iit = m_globs.begin(); iit != m_globs.end(); ++iit) {
        if(!fnmatch(iit->first.c_str(), uri.c_str(), 0)) {
            return iit->second;
        }
    }
    return m_default;
