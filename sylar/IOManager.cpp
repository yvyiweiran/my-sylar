...
void IOManager::FdContext::triggetEvent(IOManager::Event event) {
...
IOManager::IOManager(int thread, bool use_caller, const std::string& name)
        :Scheduler(thread, name, use_caller){

    m_epfd = epoll_create(5000);
    SYLAR_ASSERT2(m_epfd > 0, "epoll_creat error");
    ...
    // SYLAR_LOG_DEBUG(g_logger) << &m_mutex;
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    // SYLAR_LOG_DEBUG(g_logger) << "IOManager::addEvent";
    ...
    if(m_fdContexts.size() > (size_t)fd) {
        ...
        while((int)m_fdContexts.size() <= fd)
...
bool IOManager::cancelAll(int fd) {
    ...
    if(fd_ctx->events & READ) {
        fd_ctx->triggetEvent(READ);
        --m_pendingEventCount;
    }
    if(fd_ctx->events & WRITE){
        fd_ctx->triggetEvent(WRITE);
        --m_pendingEventCount;
    }
