...
int64_t Socket::getSendTimeout() {
    FdCtx::ptr ctx = fdMgr::GetIstance()->get(m_sock);
    if(ctx) {
...
bool Socket::init(int sock) {
    FdCtx::ptr ctx = fdMgr::GetIstance()->get(sock);
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            SYLAR_LOG_ERROR(g_logger) << "sock=" << " connect(" << addr->toString()
                << ") errno=" << errno << " errstr=" << strerror(errno);
            close();
            return false;
        }
...
bool Socket::lisent(int backlog) {
    if(!isValid()) {
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        SYLAR_LOG_ERROR(g_logger) << "gerpeername error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
