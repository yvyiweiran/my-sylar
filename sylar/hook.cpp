...
static ssize_t do_io(int fd, OriginFun fun, const char* hook_fun_name
    ...
    sylar::FdCtx::ptr ctx = sylar::fdMgr::GetIstance()->get(fd);
    if(!ctx) {
        return fun(fd, std::forward<Args>(args)...);
    }
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
    ...
    long timeout_ms = req->tv_sec * 1000 + req->tv_nsec / 100 / 100;
    sylar::Fiber::ptr fiber = sylar::Fiber::GetThis();
    sylar::IOManager* iom = sylar::IOManager::GetThis();
...
int socket(int domain, int type, int protocol) {
    ...
    sylar::fdMgr::GetIstance()->get(fd, true);
    return fd;
}
...
int connect(int sockfd, const struct sockaddr *serv_addr, socklen_t addrlen){
    return connect_with_timeout(sockfd, serv_addr, addrlen, sylar::s_connect_timeout);
}
...
int ioctl(int d, unsigned long int request, ...) {
    ...
    if(FIONBIO == request) {
        ...
        sylar::FdCtx::ptr ctx = sylar::fdMgr::GetIstance()->get(d);
        if(!ctx || ctx->isClose() || ctx->isSocket()) {
...
int setsockopt(int sockfd, int level, int optname, const void *optval, socklen_t optlen) {
    ...
    if(level == SOL_SOCKET) {
        if(optname == SO_RCVTIMEO || optname == SO_SNDTIMEO) {
            sylar::FdCtx::ptr ctx = sylar::fdMgr::GetIstance()->get(sockfd);
            if(ctx) {
                const timeval* v = (const timeval*)optval;
                ctx->setTimeout(optname, v->tv_sec * 1000 + v->tv_usec / 100);
            }
        }
