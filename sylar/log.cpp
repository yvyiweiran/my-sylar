...
LogLevel::Level LogLevel::ToLevel( std::string level) {
    switch(level[0]) {
        case 'D':
        case 'd':
            return DEBUG;
...
{ \
    #str, []( const std::string \
        &fmt ) { return FormatItem::ptr( new C( fmt ) ); } \
}
...
public:
    std::set<LogDefine> operator() (const std::string& v) {
        ...
        for(size_t i = 0; i < node.size(); ++i) {
            ...
            ld.level = LogLevel::ToLevel(n["level"].IsDefined()
            ...
            if(n["appender"].IsDefined()) {
                for(size_t x = 0; x < n["appender"].size(); ++x) {
                    ...
                    if(type == "FileLogAppender") {
                        lad.type = 1;
                        if(!a["file"].IsDefined()) {
                            std::cout << "log config error: fileappender type is null, "
                                      << a << std::endl;
                            continue;
                        }
                ...
                for(auto& a : i.appenders) {
                    ...
                    if(!a.formatter.empty()) {
                        ...
                        if(!fmt->isError()){
                            ...
                            std::cout << "appender name=" << i.name << " formatter="
